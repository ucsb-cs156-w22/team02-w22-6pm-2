...
public class UCSBSubjectControllerTests extends ControllerTestCase {
    ...
    // @WithMockUser(roles = { "USER" })
    // @Test
    // public void api_UCSBSubject__user_logged_in__search_for_todo_that_belongs_to_another_user() throws Exception {

    //     // arrange

    //     User u = currentUserService.getCurrentUser().getUser();
    //     User otherUser = User.builder().id(999L).build();
    //     //UCSBSubject otherUsersUCSBSubject = UCSBSubject.builder().title("UCSBSubject1").details("UCSBSubject 1").done(false).user(otherUser).id(13L)
    //             .build();

    //     when(ucsbSubjectRepository.findById(eq(13L))).thenReturn(Optional.of(otherUsersUCSBSubject));

    //     // act
    //     MvcResult response = mockMvc.perform(get("/api/UCSBSubjects?id=13"))
    //             .andExpect(status().isBadRequest()).andReturn();

    //     // assert

    //     verify(ucsbSubjectRepository, times(1)).findById(eq(13L));
    //     String responseString = response.getResponse().getContentAsString();
    //     assertEquals("UCSBSubject with id 13 not found", responseString);
    // }

    // @WithMockUser(roles = { "ADMIN" })
    // @Test
    // public void api_ucsbSubject__admin_logged_in__search_for_todo_that_belongs_to_another_user() throws Exception {
    ...
    @WithMockUser(roles = { "ADMIN" })
    @Test
    public void api_UCSBSubjects__admin_logged_in__search_for_UCSBSubjects_that_does_not_exist() throws Exception {
        ...
        // act
        MvcResult response = mockMvc.perform(get("/api/UCSBSubjects/admin?id=29"))
                .andExpect(status().isBadRequest()).andReturn();

        // assert

        verify(ucsbSubjectRepository, times(1)).findById(eq(29L));
        String responseString = response.getResponse().getContentAsString();
        assertEquals("UCSBSubject with id 29 not found", responseString);
    }
    ...
    //     UCSBSubject todo1 = .builder().title("Todo 1").details("Todo 1").done(false).user(u1).id(1L).build();
    ...
    //     UCSBSubject expectedTodo = UCSB.builder()
    //             .title("Test Title")
    ...
    @WithMockUser(roles = { "ADMIN" })
    @Test
    public void api_UCSBSubject__admin_logged_in__cannot_delete_UCSBSubject_that_does_not_exist() throws Exception {
        // arrange
